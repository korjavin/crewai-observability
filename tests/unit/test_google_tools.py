...
def test_gmail_reader_tool_with_messages(monkeypatch):
    ...
    mock_gmail_data = {
        ...
        'get': get_mock_email_content(subject="Test Subject", body="This is a test body.")
    }
...
def test_google_calendar_search_tool_with_busy_slots(monkeypatch):
    ...
    # Arrange
    mock_google_auth(monkeypatch)
    busy_slots = [{'start': '2024-09-01T10:00:00Z', 'end': '2024-09-01T11:00:00Z'}]
    mock_calendar_data = {'query': get_mock_freebusy_query_response(busy_slots=busy_slots)}
    mock_google_service_build(monkeypatch, 'calendar', mock_calendar_data)

    # Act
    result = google_calendar_search_tool.run(start_time="2024-09-01T09:00:00Z", end_time="2024-09-01T17:00:00Z")
...
def test_google_calendar_search_tool_with_no_busy_slots(monkeypatch):
    ...
    # Arrange
    mock_google_auth(monkeypatch)
    mock_calendar_data = {'query': get_mock_freebusy_query_response(busy_slots=[])}
    mock_google_service_build(monkeypatch, 'calendar', mock_calendar_data)
...
def test_google_calendar_writer_tool_success(monkeypatch):
    ...
    # Arrange
    mock_google_auth(monkeypatch)
    mock_calendar_data = {'insert': get_mock_event_insert_response(event_id="evt123")}
    mock_google_service_build(monkeypatch, 'calendar', mock_calendar_data)
    event_details = {
